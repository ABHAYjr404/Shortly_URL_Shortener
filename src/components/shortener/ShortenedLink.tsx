...
export default function ShortenedLink({
  ...
  return (
    <div className="flex flex-col md:flex-row shadow-md bg-white justify-between md:items-center w-[88%] rounded-md mx-auto md:py-4 md:px-8">
      {/* 
        Add Tailwind's `truncate` class to ensure long URLs are truncated with an ellipsis.
        You can also set a max width if necessary.
      */}
      <p className="p-4 md:p-0 truncate max-w-[300px] md:max-w-full">{linkEntered}</p>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:items-center p-4 md:p-0">
        <Link className="text-cyan truncate max-w-[300px]" href={resolvedShortened}>
          {resolvedShortened}
        </Link>
        <button
          className={`${
